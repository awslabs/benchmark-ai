...
def kafka_consumer(benchmark_event: BenchmarkEvent):
    ...
    consumer.poll = Mock(
        return_value={
            CONSUMER_TOPIC: [MockConsumerRecord(value=benchmark_event, key=SOME_KEY)]
        }
    )
    return consumer
...
def simple_kafka_service(kafka_consumer: KafkaConsumer, kafka_producer: KafkaProducer):
    ...
    kafka_service = KafkaService(
        SERVICE_NAME, VERSION, PRODUCER_TOPIC, callbacks, kafka_consumer, kafka_producer
    )
...
def test_dont_add_to_running(simple_kafka_service: KafkaService):
    ...
    with pytest.raises(
        KafkaService.LoopAlreadyRunningException,
...
def test_kafka_service_stop_before_run(simple_kafka_service: KafkaService):
    ...
    with pytest.raises(
        KafkaService.LoopNotRunningException,
...
def test_invalid_message_ignored(
    kafka_consumer_with_invalid_message: KafkaConsumer, kafka_producer: KafkaProducer
):
    mock_callback = Mock(spec=KafkaServiceCallback)

    kafka_service = _create_kafka_service(
        [mock_callback], kafka_consumer_with_invalid_message, kafka_producer
    )
...
def test_message_passed_through(
    kafka_consumer: KafkaConsumer, kafka_producer: KafkaProducer
):
    ...
    kafka_service = _create_kafka_service(
        mock_callbacks, kafka_consumer, kafka_producer
    )
    kafka_service.run_loop()
...
def test_message_sent(
    ...
    expected_event.visited.append(
        VisitedService(SERVICE_NAME, timestamp=VISIT_TIME_MS, version=VERSION)
    )
    ...
    kafka_service = _create_kafka_service(
        [mock_callback], kafka_consumer, kafka_producer
    )
    kafka_service.run_loop()
...
def _create_kafka_service(callbacks, kafka_consumer, kafka_producer):
    class StopKafkaServiceCallback(KafkaServiceCallback):
        def handle_event(
            self, event: BenchmarkEvent, kafka_service: KafkaService
